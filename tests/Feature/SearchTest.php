...
{
    /**
     * Test whether the cache system works properly
     *
     * @return void
     * @throws \Spatie\DataTransferObject\Exceptions\UnknownProperties
     */
    public function test_search_cache()
    {
        ...
        $random =  'hello5977407321';
        $key = 'search.'.$random;
        $showListDecoded = json_decode($showListJson, true);

        $showListCollection = collect();
        foreach ($showListDecoded as $showList) {
            $showList = new ShowListDTO($showList);
            $showListCollection->push($showList);
        }

        Cache::forget($key);
        Cache::put($key, $showListCollection);

        $response = $this->get('api/v1?q='.$random);
        $response->assertOk();
